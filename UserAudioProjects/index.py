...
def post():
    ...
    #file = flask.request.form.get('name')
    print(file)
    ...
    cur.execute("INSERT INTO audio (filename, duration) VALUES (?, ?)",(file.filename,duration))
    connection.commit()
    connection.close()
...
def download():    
    ...
    duration_filter = flask.request.args.get('maxduration', default=-1, type=int)
